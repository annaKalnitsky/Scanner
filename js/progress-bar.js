...
(function (window) {
  function initProgressBar() {
    ...
    // this delay is required as stop animation
    var stepDelay = window.stepDelay = 0;
    ...
    var createStatusBar = function(stages, stageWidth, currentStageIndex) {
      ...
      var animationDelay = renderingWaitDelay + ProgressBar.singleStepAnimation + stepDelay;

      setTimeout(function() {
        currentStatus.style.width = (100)/(stages.length - 1)+'%';
        currentStatus.style.transition = 'width '+(ProgressBar.singleStepAnimation)+'ms linear';
      }, renderingWaitDelay);

      for (var index = 2; index < stages.length; index++) {
        setTimeout(function(index) {
          console.log(currentStatus.style.width, currentStatus.style.transition, index);
          document.querySelector('.current-status').style.width = (100 * index)/(stages.length - 1)+'%';
          document.querySelector('.current-status').style.transition = 'width '+(ProgressBar.singleStepAnimation)+'ms linear';
        }, animationDelay, index);
        animationDelay+= ProgressBar.singleStepAnimation + stepDelay;
      }
